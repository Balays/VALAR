...
bam.all.filtered <- bam.all[ ex_nr == 1, 
                             .(seqnames, qname, aln_ID, strand, start, end, cigar, seq, tag.l3, tag.r3, sample) ] # 
...
# Number of correct 3-prime adapters per strand, according to LoRTIA
bam.all.correct[,tag.l3.type := gsub('.*,', '', tag.l3)]
bam.all.correct[,tag.r3.type := gsub('.*,', '', tag.r3)]
bam.all.correct[,.N,by=.(strand, tag.r3.type, tag.l3.type)]
...
# Filter for those that have a 3-prime SoftClip
bam.sc.correct <- copy(bam.all.correct)#[!is.na(softclip_length), ]
bam.sc.correct[is.na(softclip_length), softclip_length := 0]

# get sc seq (+check_in_match from mapped [10 or 15]) for these
check_in_match <- 0
bam.sc.correct <- bam.sc.correct[,
                                   sc_seq := fifelse(
                                     strand == "-" & !is.na(softclip_length) & softclip_length > 0,
                                     paste0(
...
bam.sc.correct[, score.r3 := as.numeric(gsub(',.*', '', tag.r3))] 

bam.sc.correct[, score.l3 := as.numeric(gsub(',.*', '', tag.l3))] 


#bam.sc.correct[, tag.r3   := gsub('.*,', '', tag.r3)] 

#bam.sc.correct[, tag.l3   := gsub('.*,', '', tag.l3)] 


bam.sc.correct[, polya_sc_len := fifelse(strand == '+', str_count(sc_seq, 'A'), str_count(sc_seq, 'T'))]



bam.sc.correct[, tag := fifelse(strand == '+',  tag.r3.type, tag.l3.type)]
