...
outdir  <- 'PRV-MDBIO-4cell_whost'; try({ dir.create(outdir) })
...
if(!load.config) {
  ...
  ### Path for bamfiles
  bamdir  <- "D:/data/PRV_3cell/rebasecall/mapped_v6_whost/LoRTIA_v1/PK-15" 
...
if (!load.config) {
  config <- list(
    ...
    genome = 'LT934125.1',
    fasta_ref = '/mnt/d/data/genomes/Sus_scrofa.Sscrofa11.1.and.LT934125.1.fasta',
    virus = 'PRV-MDBIO',
    gff_file = '/mnt/d/data/genomes/Sus_scrofa.Sscrofa11.1.104.gff3',
    create.ann.from.gff = F,
    feature.df.file = "refgenome/LT934125.1_feature.df.tsv",
    ## reference transcripts
    viral.ref.gff = "/mnt/d/data/genomes/Sus_scrofa.Sscrofa11.1.104.gff3",
...
# bam.filt[qname]
bam.filt <- bam.all [!is.na(seqnames)]
bam.filt <- bam.filt[seqnames == config$genome]
...
#### RUN ADAPTER FINDER
#### BECAUSE WRONG ADAPTERS WERE USED IN THE LoRTIA SCRIPT !
...
fwrite(bam.sc.correct, file.path(outdir, 'bam.sc.correct.tsv.gz'), sep = '\t')

## adapter distance from the start of mapping
bam.sc.correct <- fread(file.path(outdir, 'bam.sc.correct.tsv.gz'), sep = '\t')
