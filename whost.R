...
if (config$create.ann.from.gff) {
  ...
  setDT(gff)
  CDS.df       <- gff[gff$type == 'gene', c("seqnames", "start", "end", "strand", "type", "Name", "ID", "biotype")]
  CDS.df$gene  <- CDS.df$Name
  
  feature.df <- CDS.df[,c("seqnames", "start", "end", "strand", "type", "gene", 'ID')]
  ...
  fwrite(feature.df, config$feature.df.file, sep = '\t')
  fwrite(gff, paste0(config$gff_file, '.tsv.gz'), sep = '\t')
  
} else {
  feature.df <- fread(config$feature.df.file)
  feature.colname <- 'ID'
}
