...
```{r, eval=T}
gene.sample_count.sp <- fread(paste0(res.dir, '/gene.sample.count.sp.tsv'), na.strings = '')
kin_classes          <- unique(gene.sample_count.sp[, .(gene, Kinetic_class)])


### Cluster Tags of dominant Clusters for each Gene from TSSr output
##
assignedClusters <- fread(paste0(outdir, '/TSSr.TSS.dcDNA.best.assignedClusters.txt'))
#assignedClusters$Kinetic_class <- NA
assignedClusters$gene_cluster  <- NA
assignedClusters[, seqnames := chr]
assignedClusters[, gene := str_to_upper(gene)]
assignedClusters <- merge(kin_classes, assignedClusters, by='gene', all.y=T)
#
assignedClusters.sp <- dcast(assignedClusters, seqnames + strand + gene + gene_cluster + Kinetic_class ~ group, value.var = 'tags', fill=0)

metacols <- c('hpi', 'Time', 'cell_line', 'group')
metafilt <- unique(assignedClusters[, ..metacols]) #$sample <- [,metacols]
metafilt[, sample := group]
...
#### Normalization Bases
 ...
 if (norm.method == 'average_genome') {
  ...
  #normBase <- fread('../rebasecall/read_count.txt')
  normBase <- data.table(sample = unique(orf.sum$sample))
...
orf.sum.perc[ , farthest_rep := ifelse(farthest_from_mean, gsub('.*_', '', sample), NA), by=.(sample, gene)]
...
```{r, fig.width=16, fig.height=10, eval=F}
# Samples
...
# Groups
countData <- orf.sum.sp.mat[, grepl('PK', colnames(orf.sum.sp.mat)) ]
