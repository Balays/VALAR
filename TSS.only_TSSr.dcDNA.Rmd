...
```{r, message=FALSE, warning=FALSE, cache=FALSE, echo=FALSE, eval=F}
#### Import TSS counts
...
multip      <- 10000
 ...
 if (norm.method == 'average_genome') {
  ...
  #normBase <- fread('../rebasecall/read_count.txt')
  normBase <- data.table(copy(metafilt))
...
```{r, eval=F}
### Filtering of very-low coverage samples --> already done
...
orf.sum.perc <- orf.sum.perc[,.(read_count=sum(read_count)), 
                             by=.(seqnames, strand, gene, norm_base, sample, group, hpi, Time, cell_line, Kinetic_class)]
...
plot_data <- ab.merged.gt[norm.method == norm.method
