...
prime3.counts[, group     := paste(cell_line, hpi, sep='_')]
...
fwrite(TES.raw.dt, paste0(outdir, '/TSSr.TES.dcDNA.raw.txt'), sep='\t')
...
fwrite(TES.dt, paste0(outdir, '/TSSr.TES.dcDNA.processed.txt'), sep='\t')
...
setnames(tagClusters, colnames(tagClusters), gsub('tss', 'tes', colnames(tagClusters)) )


fwrite(tagClusters, paste0(outdir, '/TSSr.TES.dcDNA.all.tagClusters.txt'), sep='\t')
...
setnames(ConsClusters, colnames(ConsClusters), gsub('tss', 'tes', colnames(ConsClusters)) )


fwrite(ConsClusters, paste0(outdir, '/TSSr.TES.dcDNA.all.ConsClusters.txt'), sep='\t')
...
setnames(ClusterShapes, colnames(ClusterShapes), gsub('tss', 'tes', colnames(ClusterShapes)) )
...
setnames(enhancers.dt, colnames(enhancers.dt), gsub('tss', 'tes', colnames(enhancers.dt)) )
...
TC_summary <- TSSr_clusters[, .(
  TC.start = min(start), 
  TC.end   = max(end),   
  consensus_peak   = round(mean(dominant_tes),0),  # or weighted average if desired
  min_peak    = min(dominant_tes),
  max_peak    = max(dominant_tes),
...
# Peak shifts
TSSr_clusters[, peak_shift := consensus_peak - dominant_tes, by = .(chr , strand, hpi, cell_line, cluster)]
