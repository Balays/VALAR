...
alpha <- data.frame(cov_geom        = 0.75,
                    gene_geom       = 0.85,
                    unstranded_geom = 0.85)
...
#### coverage data
prime        <- 'prime5'
plot.data    <- data.table(data.frame(prime5.counts))
...
try({
  ...
  #cagefr.clust            <- cagefr.clust[clusterType == 'TSS', ]
...
plotfun.all   <- function(samples=NA) {
  ...
  FigY    <- plot.genome.region(visfrom=visfrom,	visto=visto, samples = samples,
                                ...
                                geom    = geom_area(aes(x=prime5, y = count, fill=strand, color = strand), position = 'identity', alpha=alpha$cov_geom),
                                #geom    = geom_point(aes(x=prime5, y = count, color=strand), alpha=alpha$cov_geom, size=0.1),
