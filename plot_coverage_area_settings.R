...
#### coverage data
prime        <- 'pos'
plot.data    <- data.table(data.frame(cov.counts))
#setnames(plot.data, old=c('pos', 'N'), new=c('prime5', 'count'), skip_absent=TRUE)
#plot.data[,start := prime5][,end := prime5][,position := prime5][order(seqnames, strand)]

plot.data <- as.data.frame(plot.data)
plot.data[,'correct_cov'] <- T

if(prime == 'prime5') {correct_prime <- 'correct_tss'} else if (prime == 'prime3') {correct_prime <- 'correct_tes'} else if (prime == 'pos') {correct_prime <- 'correct_cov'}
...
try({
  ...
  cagefr.clust            <- cagefr.clust[!is.na(CAGE_significance),]
  ## filter for **
  cagefr.clust            <- cagefr.clust[CAGE_significance != '*', ]

})
cagefr.clust              <- cagefr.clust[strand %in% c('+', '-'), ]
if(is.null(cagefr.clust)) { add.cageTSS <- F } else { add.cageTSS <- T }
...
bin_width  <- 50
breakseq   <- 5000
#ylim       <- c(0, 5000) # NULL # c(-1000, 1000)
ylims.gene <- c(-2, 3.5) #NULL # c(10, -20) ##
scales     <- 'free_y'

genomplot.scale <- 7

plotfun   <- function(samples) {
  ...
  FigY.p    <- plot.genome.region(visfrom=visfrom,	visto=visto, samples = samples,
                                  ...
                                  geom    = geom_area(aes(x=prime5, y = count, fill=strand), position = 'identity', alpha=alpha$cov_geom, color = "black"),
  ...
  FigY.ann    <- plot.genome.region(visfrom=visfrom,	visto=visto, samples = samples,
                                    ...
                                    add.feature=F,
                                    force.all.gene.down=T, angle = angle,
                                    add.cageTSS=T, cagefr.clust=cagefr.clust, cagefr.clust.dist=3,
...
plotfun.all   <- function(samples=NA) {
  ...
  FigY    <- plot.genome.region(visfrom=visfrom,	visto=visto, samples = samples,
                                ...
                                geom    = geom_area(aes(x=pos, y = count, fill=strand), color = 'black', position = 'identity', alpha=alpha$cov_geom),
                                #geom    = geom_point(aes(x=prime5, y = count, color=strand), alpha=alpha$cov_geom, size=0.1),
                                ...
                                #labels=labs(fill = "Strand"),
                                vline=NULL,
                                xlab_name='Genomic Position', ylab_name='Coverage',
                                return.plot.data=F)
